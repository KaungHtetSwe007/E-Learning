<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<title>instructor profile</title>
	<link rel="stylesheet" type="text/css" href="css/style1.css">
	<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css" integrity="sha512-DTOQO9RWCH3ppGqcWaEA1BIZOC6xxalwEsw9c2QQeAIftl+Vegovlnee1c9QX4TctnWMn13TZye+giMm8e2LwA==" crossorigin="anonymous" referrerpolicy="no-referrer" />
</head>
<body>
	<div class="header_wrapper">
		<header></header>
		<div class="cols_container">
			<div class="left_col">
				<div class="img_container">
					<img src="https://yt3.googleusercontent.com/ytc/AIf8zZTNiWLD2-Qw1eWFmRJkGBzfI-8oFNmYk5tBnpNurQ=s176-c-k-c0x00ffffff-no-rj" alt="mountain">
					<span></span>
				</div>
				<div class="about">
				<h2>CodeWithHarry</h2>
				<h4>Java ဆရာ</h4>
				<blockquote>Code With Harry is my attempt to teach basics and those coding techniques to people in short time which took me ages to learn.
</blockquote></div>
				<div class="content">
					<ul>
						<li><a href="https://www.youtube.com/@CodeWithHarry"><i class="fa-brands fa-youtube"></i></a></li>
						<li><a href="https://www.facebook.com/CodeWithHarry"><i class="fa-brands fa-facebook"></i></a></li>
						<li><a href="https://www.youtube.com/@CodeWithHarry"><i class="fa-solid fa-globe"></i></a></li>
						<li><a href="https://www.instagram.com/codewithharry "><i class="fa-brands fa-instagram"></i></a></li>
					</ul>
				</div>
			</div>
			<div>
			<main class="cards">
        <section class="card contact">
            <div class="icon">
                <img src="data:image/png;base64," alt=".">
            </div>
            <h3>Java</h3>
            <span>Java သည် multiplatform  class-based,object-oriented  programming language ဖြစ်သည်။</span>
            <button><a href="https://youtu.be/ntLJmHOJ0ME?si=EykScwgFtdBtIiWb">Watch Now</a></button>
        </section>
        <section class="card shop">
            <div class="icon">
                <img src="https://cdn.pixabay.com/photo/2017/08/05/11/16/logo-2582748_960_720.png" alt="Shop here.">
            </div>
            <h3>HTML</h3>
            <span>HTML သည် websiteများကို ဖန်တီးရန်အတွက်  markup language တစ်ခုဖြစ်သည်။</span>
            <button><a href="https://youtu.be/BsDoLVMnmZs?si=ryuNyT0C0FZ04xGA">Watch Now</a></button>
        </section>
        <section class="card about">
            <div class="icon">
                <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQYLQSLRj7ynwV9b9u4fSI4RsFhY1e-KhGSW9PJWhMariu1oZp4H4njkiubKuDDEvenI2o&usqp=CAU" alt="About us.">
            </div>
            <h3>Python</h3>
            <span>Python သည်  interpreted,high-level programming languageဖြစ်သည်။</span>
            <button><a href="https://youtu.be/gfDE2a7MKjA?si=BJkk9E3Zmy6wyG9g">Watch Now</a></button>
        </section>
    </main>

</body>
</html>