<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<title>instructor profile</title>
	<link rel="stylesheet" type="text/css" href="css/style1.css">
	<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css" integrity="sha512-DTOQO9RWCH3ppGqcWaEA1BIZOC6xxalwEsw9c2QQeAIftl+Vegovlnee1c9QX4TctnWMn13TZye+giMm8e2LwA==" crossorigin="anonymous" referrerpolicy="no-referrer" />
</head>
<body>
	<div class="header_wrapper">
		<header></header>
		<div class="cols_container">
			<div class="left_col">
				<div class="img_container">
					<img src="https://yt3.googleusercontent.com/ytc/AIf8zZQJUhLAbfTEjP6QrFxNozUziJRiZL_v30aK2Xg1=s176-c-k-c0x00ffffff-no-rj" alt="mountain">
					<span></span>
				</div>
				<div class="about">
				<h2> ProgrammingKnowledge </h2>
				<h4>Python ဆရာ</h4>
				<blockquote>ဟိုင်း,
ကျွန်ုပ်၏ဗီဒီယိုတွင် ကျွန်ုပ်အသုံးပြုသည့် ကျွန်ုပ်၏ကုဒ်ကို မျှဝေရန် အမြဲတောင်းဆိုထားသည်။ လူတွေရဲ့မေးခွန်းတွေကို ဖြေဆိုရတာ အရမ်းကောင်းပြီး ပြဿနာတစ်ခုကို ဖြေရှင်းလိုက်တဲ့အခါ သင်ခံစားရတဲ့ ခံစားချက်က အမြဲတမ်းကောင်းပါတယ်။
</blockquote></div>
				<div class="content">
					<ul>
						<li><a href="https://www.youtube.com/@ProgrammingKnowledge"><i class="fa-brands fa-youtube"></i></a></li>
						<li><a href="https://www.facebook.com/ProgrammingKnowledgeLearning/"><i class="fa-brands fa-facebook"></i></a></li>
						<li><a href="https://scholar.google.com/citations?user=jtEN4U8AAAAJ"><i class="fa-brands fa-instagram"></i></a></li>
						<li><a href="https://twitter.com/ProgrammingKnow"><i class="fa-brands fa-twitter"></i></a></li>
					</ul>
				</div>
			</div>
			<div>
			<main class="cards">
        <section class="card contact">
            <div class="icon">
                <img src="data:image/png;base64," alt=".">
            </div>
            <h3>Java</h3>
            <span>Java သည် multiplatform  class-based,object-oriented  programming language ဖြစ်သည်။</span>
            <button><a href="https://youtu.be/ioPagXHCRjc?si=UWLjoEFAXd6iZxpR">Watch Now</a></button>
        </section>
        <section class="card shop">
            <div class="icon">
                <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/768px-JavaScript-logo.png" alt="Shop here.">
            </div>
            <h3>Javascript</h3>
            <span>JavaScript သည် dynamic website ဖန်တီးရန် scripting languageဖြစ်သည်။</span>
            <button><a href="https://youtu.be/_LtwT5_zmDs?si=1r8V8EF1QicKNfOG">Watch Now</a></button>
        </section>
        <section class="card about">
            <div class="icon">
                <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQYLQSLRj7ynwV9b9u4fSI4RsFhY1e-KhGSW9PJWhMariu1oZp4H4njkiubKuDDEvenI2o&usqp=CAU" alt="About us.">
            </div>
            <h3>Python</h3>
            <span>Python သည်  interpreted,high-level programming languageဖြစ်သည်။</span>
            <button><a href="https://youtu.be/kdLM6AOd2vc?si=kRJrzmdtgX1oh8NA">Watch Now</a></button>
        </section>
    </main>

</body>
</html>